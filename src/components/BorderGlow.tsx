...
const BorderGlow = ({
  ...
  useEffect(() => {
    ...
    const card = cardRef.current;
    const sweepClass = styles.sweepActive;
    const angleStart = 110;
    const angleEnd = 465;
    card.classList.add(sweepClass);
    ...
    animateValue({ ease: easeInCubic, delay: 2500, duration: 1500, start: 100, end: 0,
      ...
      onEnd: () => card.classList.remove(sweepClass),
    });
  }, [animated]);
  ...
  return (
    <div
      ...
      className={`${styles.borderGlowCard}${lightSurface ? ` ${styles.borderGlowCardLight}` : ''} ${className}`.trim()}
      style={{
